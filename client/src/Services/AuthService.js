...
export default {
  register: async (user) => {
    try {
      const res = await axios(`/api/user/register`, {
        method: "post",
  ...
  login: async (user) => {
    try {
      const res = await axios(`/api/user/login`, {
        method: "post",
        credentials: "include",
        body: JSON.stringify(user),
        headers: {
  ...
  isAuthenticated: async () => {
    try {
      const res = await axios(`/api/user/authenticated`);
      if (res.status !== 401) return res.json().then((data) => data);
      else return { isAuthenticated: false, user: { email: "" } };
    } catch (error) {
  ...
  getInfo: async (id) => {
    try {
      const res = await axios(`/api/user/info/${id}`);
      if (res.status !== 401) return res.json().then((data) => data);
